...
// the export object
export var HAAR = { VERSION: "@@VERSION@@" }, Detector, Feature, proto = 'prototype', undef = undefined;
    ...
    * * _CanvasClass_ : This is optional and used only when running in node (passing the node-canvas object).
    ...
    image: function (image, scale, canvas) {
        ...
        if (image) {
            ...
            // temp-fix: 
            w = self.origWidth = isVideo ? image.offsetWidth : image.width;
            h = self.origHeight = isVideo ? image.offsetHeight : image.height;
